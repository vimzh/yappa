...
const conversationGuide = `Write 24-44 short turns and roughly 850-1,100 spoken words. Each turn should be 1-3 sentences. The speakers must answer each other directly, challenge assumptions, concede strong points, and change pace naturally.
...
export async function iterateTranscript(options: {
  ...
  for (let iteration = 1; iteration <= options.maxIterations; iteration += 1) {
    ...
    if (iteration >= 3 && review.approved) {
      break;
    }
  }
